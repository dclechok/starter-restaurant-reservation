...
function validateTableData(req, res, next) {
  ...
  if (!table_name || table_name.length < 2)
    return next({
      status: 400,
      message: "Table must have valid table_name.",
    });
...
async function reservationInReservations(req, res, next) {
  ...
  if (!reservation)
    return next({
      status: 404,
      message: `Reservation with the ID of ${reservation_id} does not exist.`,
    }); //the reservation with id doees not exist
  next();
}
...
async function capacityCheck(req, res, next) {
  ...
  const { capacity, reservation_id } = await knex
    .from("tables")
    .where("table_id", table_id)
    .then((table) => table[0]); //fetch specific table's capacity
...
async function tableOccupied(req, res, next) {
  ...
  const { reservation_id } = await knex
    .from("tables")
    .where("table_id", table_id)
    .then((table) => table[0]); //fetch specific table's capacity
  const query = await knex("reservations")
    .where("reservation_id", reservation_id)
    .select("status")
    .then((res) => res[0]);
  res.locals.theResId = reservation_id;
  if(!reservation_id) return next({ status: 400, message: "Table is not occupied." });
...
async function checkAlreadySeated(req, res, next) {
  ...
  if (data.status === "seated")
    return next({
      status: 400,
      message: "Table status is already seated or occupied.",
    });
  next();
}
...
async function tableExists(req, res, next) {
  ...
  if (query.length === 0)
    return next({ status: 404, message: `Table ${table_id} is non-existant.` }); //table id exist in Tables?
  next();
}
...
async function create(req, res) {
  ...
  const data = await knex("tables")
    .insert(result)
    .returning("*")
    .then((results) => results[0]); //insert body data into tables
...
async function update(req, res) {
  ...
  await knex("tables")
    .where("table_id", table_id)
    .update("reservation_id", reservation_id); //also up reservation_id in tables
...
module.exports = {
  ...
  delete: [tableExists, tableOccupied, asyncErrorBoundary(deleteTable)],
};
